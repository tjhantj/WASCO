...
def iteration_nodes_upperbound(G_prime, candidate_nodes, s, b, t, spent, coreness, upperbound, s_cand, FT, T1_self_edge, T3_reuse, delta_Tactic, comp_of={}, best=(None, 0, 0.0, 0)):
    ...
    for i in range(c):
        ...
        for j in range(i if T1_self_edge else i+1, c):
            v = candidate_nodes[j]
            if T3_reuse:
                if comp_of[u] == comp_of[v]:    # we considered selfedge in cache
                    continue
            e = (u, v)
...
def save_result_to_csv(A, s_core_num, total_time, memory, args, total_follower):
    write_header = not os.path.exists(args.output_path)
    data = args.network.split('/')[4]
    with open(args.output_path, 'a', newline='') as f:
        writer = csv.writer(f)
        if write_header:
            writer.writerow(["data", "s", "b", "tactics", "num_s_core", "follower", "num_anchored_edges", "iter", "delta", "num_follower", "total_time", "memory"])
        for idx, (_, delta, _, num_follower) in enumerate(A, start=1):
            writer.writerow(
                [data, args.s, args.b, args.tactics, s_core_num, total_follower, len(A), idx, delta, num_follower, total_time, memory])
