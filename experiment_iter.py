...
def run(G, s, b, t, T1_self_edge = True, T2_upperbound = True, T3_reuse = True, delta_Tactic = "compute"):
    ...
    if T1_self_edge:
        non_s_core, s_cand = exp_func_iter.self_edge_pruning(G_prime) # pruned set
    ...
    while spent < b:
        ...
        if T1_self_edge:
            # 1. Making Candidate
            candidate_nodes = exp_func_iter.make_candidate_nodes(G_prime, non_s_core, s, b-spent, coreness, upperbound, UT, T2_upperbound, delta_Tactic)
            # 2. Iterating Candidate to find best edge
            if T2_upperbound:
                # tactics TT
                best = exp_func_iter.iteration_nodes_upperbound(G_prime, candidate_nodes, s, b, t, spent, coreness, upperbound, s_cand, FT, T1_self_edge, T3_reuse, delta_Tactic, comp_of, best)
            else:
                # tactics TF
                best = exp_func_iter.iteration_nodes_no_upperbound(G_prime, candidate_nodes, s, b, t, spent, coreness, s_cand, FT, T3_reuse, delta_Tactic, comp_of, best)
        else:
            if T2_upperbound:
                # tactics FT
                candidate_nodes = exp_func_iter.make_candidate_nodes_v2(G_prime, G_prime.nodes, s, b-spent, coreness, upperbound, UT, T2_upperbound, delta_Tactic)
                best = exp_func_iter.iteration_nodes_upperbound(G_prime, candidate_nodes, s, b, t, spent, coreness, upperbound, s_cand, FT, T1_self_edge, T3_reuse, delta_Tactic, comp_of, best)
            else:
                # tactics FF
                candidate_edges = exp_func_iter.make_candidate_edges(G_prime, G_prime.nodes, s, b-spent, coreness)
                best = exp_func_iter.iteration_edges_no_upperbound(G_prime, candidate_edges, s, b, t, spent, coreness, FT, T3_reuse, delta_Tactic, comp_of, best)
        ...
        A.append(((u,v), best_delta, most_FR, most_follower, self, reinforce))
        ...
        if T3_reuse:
            ...
            coreness = {}
            s_core_num = functions.calculate_s_core(G_prime, G_prime.nodes, s, coreness)
    return A, FT, UT, G_prime, total_follower, GLOBAL_CNT
...
def find_intra_best(G, nodes, coreness, s, t, b, spent, upperbound, T1_self_edge, T2_upperbound, UT, FT, s_cand, delta_Tactic):
    ...
    if T1_self_edge:
        candidate_nodes = exp_func_iter.make_candidate_nodes(G, nodes, s, b-spent, coreness, upperbound, UT, T2_upperbound, delta_Tactic)

        if T2_upperbound:
            # tactics TT
            best = exp_func_iter.iteration_nodes_upperbound(G, candidate_nodes, s, b, t, spent, coreness, upperbound, s_cand, FT, T1_self_edge, False, delta_Tactic)
        else:
            # tactics TF
            best = exp_func_iter.iteration_nodes_no_upperbound(G, candidate_nodes, s, b, t, spent, coreness, s_cand, FT, False, delta_Tactic)
    
    else:
        if T2_upperbound:
            # tactics FT
            candidate_nodes = exp_func_iter.make_candidate_nodes_v2(G, nodes, s, b-spent, coreness, upperbound, UT, T2_upperbound, delta_Tactic)
            best = exp_func_iter.iteration_nodes_upperbound(G, candidate_nodes, s, b, t, spent, coreness, upperbound, s_cand, FT, T1_self_edge, False, delta_Tactic)
        else:
            # tactics FF
            candidate_edges = exp_func_iter.make_candidate_edges(G, nodes, s, b-spent, coreness)
            best = exp_func_iter.iteration_edges_no_upperbound(G, candidate_edges, s, b, t, spent, coreness, FT, False, delta_Tactic)
